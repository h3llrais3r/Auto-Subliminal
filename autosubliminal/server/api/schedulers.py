...
class SchedulersApi(RestResource):
    ...
    def get(self, scheduler_name=None):
        ...
        if scheduler_name is None:
            return [autosubliminal.SCHEDULERS[scheduler].to_dict(True) for scheduler in autosubliminal.SCHEDULERS]
        elif scheduler_name in autosubliminal.SCHEDULERS:
            return autosubliminal.SCHEDULERS[scheduler_name].to_dict(True)
        else:
